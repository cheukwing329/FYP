...
[corr,lags]=xcorr(s_1,s_receive(1:2*f_speaker/bitpersec));
delay = find(corr==max(corr));
delay = abs(lags(delay));
s_rrx = s_receive((delay+1):(ceil(totalbit_tx/bitpersec)*f_speaker+delay));

%% Normalize
s_rrx(:) = s_rrx./max(s_rrx(ceil((3/10)*f_speaker/bitpersec):ceil((7/10)*f_speaker/bitpersec)));
...
for i=1:totalbit_tx
    ...
    r_1 = sum(x_1);
    r_0 = sum(x_0);
    if (r_1-r_0)>=f_speaker/bitpersec/8
        bit_rx(i) = 1;
    else
        bit_rx(i)=0;
    end
